...
export const LegalDataProvider: React.FC<LegalDataProviderProps> = ({ children }) => {
  ...
  React.useEffect(() => {
    ...
    Promise.all([
      fetch(getApiUrl('/api/cases'), { credentials: 'include' }).then(r => r.ok ? r.json() : Promise.resolve([])),
      fetch(getApiUrl('/api/clients'), { credentials: 'include' }).then(r => r.ok ? r.json() : Promise.resolve([])),
      fetch(getApiUrl('/api/alerts'), { credentials: 'include' }).then(r => r.ok ? r.json() : Promise.resolve([])),
      fetch(getApiUrl('/api/time-entries'), { credentials: 'include' }).then(r => r.ok ? r.json() : Promise.resolve([])),
      fetch(getApiUrl('/api/hearings'), { credentials: 'include' }).then(r => r.ok ? r.json() : Promise.resolve([])),
      fetch(getApiUrl('/api/invoices'), { credentials: 'include' }).then(r => r.ok ? r.json() : Promise.resolve([])),
